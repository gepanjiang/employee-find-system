			    ...
			    $("#btnDelete").click(function(){
			        ...
			        $(".check:checked").each(function(index,a){
						...
						$.dialog.confirm("你确定要删除吗?", function(){
				             window.location.href=encodeURI('chatDel.do?idlist='+ idlist);
						  });
					});
			...
			<div class="search-options">
				<form id="searchForm"
					...
					<table class="grid" cellspacing="0" width="100%">
						<tbody>
							<tr>
								<td>留言内容 <input name="key" value="${key}"
...
<a id="btnDelete" class="action-btn" href="javascript:void(0)">
			...
			<table id="module" width="100%" border="0" cellspacing="0"
				...
				<tbody>
						...
						<tr>
							<td>&nbsp;<input id="chk${chat.id}" class="check" name="chk${chat.id}" type="checkbox" value='${chat.id}'/>
							</td>
							<td>${chat.member.uname}</td>
                            <td>${chat.savetime}</td>
							<td>${chat.content}</td>
							<td>${chat.hfcontent}</td>
							<td>
								<a class="edit" href="chatedit.jsp?id=${chat.id}">回复</a>
							</td>
						</tr>
						 </c:forEach>
						 <tr>
		        <td align="center" style="font-weight: bold;font-family:楷体;font-weight: bold; color:blue" colspan="10">
                     ...
                     <a href="chatList.do?index=1&key=${key}" style="font-family:楷体;">首页</a>&nbsp;&nbsp;
                     
                     <c:choose>
                     <c:when test="${index >1}">
                     <a href="chatList.do?index=${index-1}&key=${key}" style="font-family:楷体;">上一页</a>
                     </c:when>
                     <c:otherwise>
                     ...
                     <a href="chatList.do?index=${index+1}&key=${key}" style="font-family:楷体;">下一页</a>
                     </c:when>
                     <c:otherwise>
                     ...
                     &nbsp;&nbsp;
                     <a href="chatList.do?index=${pages}&key=${key}" style="font-family:楷体;">末页</a>
                 </td>
		        </tr>
					</c:otherwise>
					</c:choose>
